 ...
 * Module Dependencies
 */
const { checkType, bodyParser } = require('../../../utils');
const compose = require('../compose');
const { HTTP_METHODS: methods } = require('../custom_http');
...
Router.prototype.dispatchRoutes = function () {
	...
	let dispatch = async function dispatch(ctx, next) {
		...
		let layersToCompose = matchedLayers.reduce(function (layerChain, layer) {
			layerChain.push(function (ctx, next) {
				ctx.params = layer.params(path, ctx.params);
				return next();
			});
			return layerChain.concat(layer.stack);
		}, []);
...
/**
 * Trims the last slash of Request url if present
 * @param {string} path Context(Request) path
 * @returns
 */
